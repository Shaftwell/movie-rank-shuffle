...
import { Film, Calendar, Edit, Eye, EyeOff, User, Star, StarHalf } from 'lucide-react';
      ...
      <div className="flex items-stretch min-h-[180px] sm:min-h-[200px] relative overflow-hidden">
        ...
        {movie.watched && (
          <div className="absolute top-3 right-3 z-20">
            <div className="bg-primary/90 text-primary-foreground rounded-full p-1.5 shadow-lg backdrop-blur-sm">
        ...
        <div className="flex-1 flex flex-col justify-between p-4 sm:p-5 md:p-6 min-w-0">
          <div className="flex-1">
            {/* Title */}
            <h3 className={cn(
              "text-lg sm:text-xl md:text-2xl font-bold mb-2 leading-tight line-clamp-2",
              "bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text",
              movie.watched && "text-muted-foreground"
            )}>
              {movie.title}
            </h3>
            ...
            {movie.director && (
              <div className="flex items-start gap-2 text-xs sm:text-sm mb-2">
                ...
                <div className="min-w-0">
                  ...
                  <span className="font-medium text-foreground/90 line-clamp-1">{movie.director}</span>
                </div>
              </div>
            )}
            ...
            {movie.actors && movie.actors.length > 0 && (
              <div className="flex items-start gap-2 text-xs sm:text-sm">
                <Star className="h-3.5 w-3.5 mt-0.5 flex-shrink-0 text-muted-foreground" />
                <div className="min-w-0">
                  <span className="text-muted-foreground/70">Starring: </span>
                  <span className="text-foreground/80 line-clamp-1">{movie.actors.join(', ')}</span>
                </div>
              </div>
            )}
          </div>
